...
import bf.colocation.immo.config.Constants;
...
public class UserService {
     ...
     * ROLE_ADMIN n'y figure JAMAIS : sans cette liste blanche, n'importe qui
     * s'inscrit administrateur en modifiant la requête depuis la console du navigateur.
     */
    /**
     * Active les comptes dès l'inscription. Vrai en dev/prod tant qu'aucun SMTP n'est branché :
     * sans cela, un compte créé ne peut jamais se connecter.
     * Faux dans le profil de test, pour que les tests JHipster d'inscription restent valides.
     */
    @Value("${application.security.auto-activate-accounts:true}")
    private boolean autoActivation;

    private static final Set<String> ROLES_AUTO_ATTRIBUABLES = Set.of(
    ...
    public User registerUser(AdminUserDTO userDTO, String password) {
        ...
        newUser.setActivated(autoActivation);
        newUser.setActivationKey(RandomUtil.generateActivationKey());

        Set<Authority> authorities = new HashSet<>();
        authorityRepository.findById(AuthoritiesConstants.USER).ifPresent(authorities::add);

        // Rôle métier demandé, uniquement s'il figure dans la liste blanche (EF-01.1)
        String roleMetier = null;
        if (userDTO.getAuthorities() != null) {
            roleMetier = userDTO
                .getAuthorities()
                .stream()
                .filter(ROLES_AUTO_ATTRIBUABLES::contains)
                .findFirst()
                .orElse(AuthoritiesConstants.UTILISATEUR);
        } else {
            roleMetier = AuthoritiesConstants.UTILISATEUR;
        }
        final String role = roleMetier;
        authorityRepository.findById(role).ifPresent(authorities::add);

        newUser.setAuthorities(authorities);
        userRepository.save(newUser);

        // Le profil métier doit exister dès l'inscription : sans ProfilDemarcheur,
        // peutPublier() renvoie false pour toujours et valider un document ne débloque rien.
        creerProfilMetier(newUser, role);

        this.clearUserCaches(newUser);
        LOG.debug("Created Information for User: {} with role {}", newUser, role);
        return newUser;
    }

    private void creerProfilMetier(User user, String role) {
        if (AuthoritiesConstants.DEMARCHEUR.equals(role)) {
            ProfilDemarcheur profilDemarcheur = new ProfilDemarcheur();
            profilDemarcheur.setUtilisateur(user);
            profilDemarcheur.setStatutValidation(StatutValidation.EN_ATTENTE);
            profilDemarcheur.setDateCreation(Instant.now());
            profilDemarcheurRepository.save(profilDemarcheur);
        } else if (AuthoritiesConstants.PROPRIETAIRE.equals(role)) {
            ProfilProprietaire profilProprietaire = new ProfilProprietaire();
            profilProprietaire.setUtilisateur(user);
            profilProprietaire.setType(TypeProprietaire.PARTICULIER);
            profilProprietaire.setDateCreation(Instant.now());
            profilProprietaireRepository.save(profilProprietaire);
        }
    }
